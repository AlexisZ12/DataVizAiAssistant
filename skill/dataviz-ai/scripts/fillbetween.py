...
def fillbetween(text_in, client, llmmodel, prompt2, prompt3, prompt4, prompt5):
    ...
    while True:
        try:
            ...
            # 检查字段名是否存在
            required_fields = ["x", "y1", "y2", "ylabel"]
            ...
            # 检查 y1 字段：必须为一维数组，元素为 int 或 float
            if not isinstance(data["y1"], list):
                raise TypeError("字段 'y1' 必须是数组类型")
            if not all(isinstance(v, (int, float)) for v in data["y1"]):
                raise TypeError("字段 'y1' 的元素必须是 int 或 float 类型")

            # 检查 y2 字段：必须为一维数组，元素为 int 或 float
            if not isinstance(data["y2"], list):
                raise TypeError("字段 'y2' 必须是数组类型")
            if not all(isinstance(v, (int, float)) for v in data["y2"]):
                raise TypeError("字段 'y2' 的元素必须是 int 或 float 类型")

            # 检查 ylabel 字段：必须为单个字符串
            if not isinstance(data["ylabel"], str):
                raise TypeError("字段 'ylabel' 必须是字符串类型")

            # 检查维度匹配
            n = len(data["x"])
            if len(data["y1"]) != n:
                raise ValueError(f"字段 'y1' 的长度({len(data['y1'])})与 'x' 的长度({n})不匹配")
            if len(data["y2"]) != n:
                raise ValueError(f"字段 'y2' 的长度({len(data['y2'])})与 'x' 的长度({n})不匹配")
    ...
    fig = MatplotlibInterface.FillBetweenInterface(data["x"], data["y1"], data["y2"], data["ylabel"],
                                                   style_config["fcolor"], style_config["falpha"],
