"""提示词：快速模式（由原 prompt/a/ 目录生成，勿手改）。"""

# Phase 1：选择图表类型
CHART_TYPE_SELECT = r"""根据用户的需求分析需要画出哪种图表，给出你的选择，根据提供的格式和要求返回成json格式
...
DATA_EXTRACT = {
    0: r"""根据用户的需求提取出你作图表需要的数据以及数据对应的标签，根据提供的格式和要求返回成json格式

### 格式：
包含3个字段x，y和ylabel，其中x为一维数组，y为二维数组，ylabel为一维数组，x和y均为int或float类型，ylabel为string类型
json数据中不要添加注释
...
### 用户需求:
""",
    1: r"""根据用户的需求提取出你作图表需要的数据以及数据，根据提供的格式和要求返回成json格式

### 格式：
包含2个字段x和y，其中x和y均为一维数组，x和y均为int或float类型
json数据中不要添加注释
...
### 用户需求:
""",
    2: r"""根据用户的需求提取出你作图表需要的数据以及数据对应的标签，根据提供的格式和要求返回成json格式
...
### 用户需求:
""",
    3: r"""根据用户的需求提取出你作图表需要的数据以及数据对应的标签，根据提供的格式和要求返回成json格式
...
### 用户需求:
""",
    4: r"""根据用户的需求提取出你作图表需要的数据以及数据对应的标签，根据提供的格式和要求返回成json格式

### 格式：
包含4个字段x，y1，y2和ylabel，其中x，y1和y2为一维数组，ylabel为单个字符串，x，y1和y2均为int或float类型，ylabel为string类型
json数据中不要添加注释
...
### 用户需求:
""",
    5: r"""根据用户的需求提取出你作图表需要的数据以及数据对应的标签，根据提供的格式和要求返回成json格式
...
### 用户需求:
""",
    6: r"""根据用户的需求提取出你作图表需要的数据以及数据对应的标签，根据提供的格式和要求返回成json格式

### 格式：
包含3个字段position，value和label，其中position为一维数组，value为二维数组，label为一维数组，value为int或float类型，position为int类型且为连续递增的整数，label为string类型
json数据中不要添加注释
...
STYLE = {
    0: r"""根据用户的需求设计图表样式，根据提供的格式和要求返回成json格式
...
### 用户需求:
""",
    1: r"""根据用户的需求设计图表样式，根据提供的格式和要求返回成json格式
...
### 用户需求:
""",
    2: r"""根据用户的需求设计图表样式，根据提供的格式和要求返回成json格式
...
### 用户需求:
""",
    3: r"""根据用户的需求设计图表样式，根据提供的格式和要求返回成json格式
...
### 用户需求:
""",
    4: r"""根据用户的需求设计图表样式，根据提供的格式和要求返回成json格式
...
### 用户需求:
""",
    5: r"""根据用户的需求设计图表样式，根据提供的格式和要求返回成json格式
...
### 用户需求:
""",
    6: r"""根据用户的需求设计图表样式，根据提供的格式和要求返回成json格式
...
# Phase 4：坐标范围
RANGE = r"""根据用户的需求和之前提取的数据设计图表坐标轴范围和刻度，根据提供的格式和要求返回成json格式
...
# Phase 5：标签
LABELS = r"""根据用户的需求和之前提取的数据设计图表的标题和坐标轴的标签，根据提供的格式和要求返回成json格式

### 格式：
包含3个字段xlabel，ylabel和title，这3个字段均为string类型
json数据中不要添加注释
...
### 当前设计参数：
{}"""
MODIFY_DATA = r"""根据用户的要求，继续修改之前的数据，返回成json格式，json的字段数量与字段名与之前的数据相同，注意修改的参数前后维数要一致
json数据中不要添加注释
