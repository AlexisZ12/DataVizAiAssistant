...
def scatter(text_in, client, llmmodel, prompt2, prompt3, prompt4, prompt5):
    ...
    while True:
        try:
            ...
            # 检查 y 字段：必须为一维数组，元素为 int 或 float
            if not isinstance(data["y"], list):
                raise TypeError("字段 'y' 必须是数组类型")
            if not all(isinstance(v, (int, float)) for v in data["y"]):
                raise TypeError("字段 'y' 的元素必须是 int 或 float 类型")

            # 检查维度匹配
            if len(data["x"]) != len(data["y"]):
                raise ValueError(f"字段 'x' 的长度({len(data['x'])})与 'y' 的长度({len(data['y'])})不匹配")
    ...
    fig = MatplotlibInterface.ScatterInterface(data["x"], data["y"],
                                               style_config["mcolor"], style_config["msize"], style_config["malpha"],
